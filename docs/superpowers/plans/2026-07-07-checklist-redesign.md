# Health Tracker v2 — Checklist Redesign Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace the form-based logging flow with a one-tap checklist: one bar per item, tap → green ✓ with auto (editable) timestamp, multi-select for Patches/Oils, no overdue styling anywhere.

**Architecture:** Same plain HTML/CSS/JS PWA, `localStorage` only. `index.html` is rewritten (Log tab and old entry form removed; new picker + edit dialogs added). `app.js` is rewritten around a per-item schedule (`{id, time, category, item|group, note}`) and multi-item entries (`{id, category, items[], timestamp, amount, scheduleId}`), with v1 entries kept readable. `styles.css` gains checklist styles and loses banner/pill/log-btn styles.

**Tech Stack:** Vanilla HTML/CSS/JavaScript, `<dialog>`, localStorage, service worker (cache-first). No build step, no test framework — verification is manual per the repo's build rules ("actually run it and show me the output"). TDD is intentionally replaced by a scripted manual verification pass (Task 5) because the spec mandates no frameworks/dependencies.

**Spec:** `docs/superpowers/specs/2026-07-07-checklist-redesign-design.md`

**Baseline safety:** branch `v1-backup` already exists on GitHub at the v1 commit.
...
**Files:**
- Modify: `index.html` (full rewrite below)

Removes: Log view, `＋ Log` tab, overdue banner div, old entry-form dialog.
Adds: `picker-dialog` (multi-select / ad-hoc / weight), `edit-dialog` (time edit + un-check + item adjust), updated welcome copy, `＋ Log something else` handled in JS (rendered into today-list, no HTML needed).

- [ ] **Step 1.1: Replace the entire contents of `index.html` with:**
...
<main id="main">
  <!-- ===== TODAY (one tappable bar per item) ===== -->
  <section id="view-today" class="view">
    <div id="today-date" class="today-date"></div>
    <div id="today-progress" class="progress-line" hidden></div>
  ...
  <section id="view-schedule" class="view" hidden>
    <p class="hint">These become the tappable bars on the Today screen. Adjust times or items anytime.</p>
...
<dialog id="welcome-dialog">
  <div class="welcome">
    ...
    <ul class="welcome-tabs">
      <li><b>Today</b> — everything for today, in time order. When you've done something, tap its bar — it gets a big green ✓ with the time. Tapped by mistake, or did it earlier? Tap the ✓ to fix the time or un-check it. Patches and Oils let you pick everything you applied at once.</li>
      <li><b>History</b> — everything you've logged, newest first. Tap any item to fix its time.</li>
      <li><b>Schedule</b> — your daily plan. Change any time or item whenever your routine changes — and add these times to your iPhone Calendar so your phone reminds you.</li>
      <li><b>More</b> — save a backup of your information, and app info.</li>
    </ul>
    <p class="welcome-note">Something extra, not on the plan? Use <b>＋ Log something else</b> at the bottom of Today. Everything you enter is saved privately on this phone. 💛</p>
...
<!-- ===== PICKER DIALOG (multi-select for group bars + "Log something else") ===== -->
<dialog id="picker-dialog">
  <form id="picker-form" method="dialog">
    <h2 id="picker-title">Log</h2>

    <div id="picker-cats" class="picker-cats" hidden></div>

    <div id="picker-items" class="picker-items"></div>

    <label class="field" id="picker-newitem-field" hidden>Add one not on the list (optional)
      <input id="picker-newitem" type="text" autocapitalize="words" placeholder="Type its name">
    </label>

    <label class="field" id="picker-weight-field" hidden>Weight
      <input id="picker-weight" type="number" inputmode="decimal" min="0" step="0.1" placeholder="e.g. 142">
    </label>

    <div class="dialog-buttons">
      <span class="spacer"></span>
      <button type="button" id="picker-cancel" class="secondary-btn">Cancel</button>
      <button type="submit" id="picker-done" class="primary-btn">Done ✓</button>
    </div>
  </form>
</dialog>

<!-- ===== EDIT DIALOG (fix time / adjust items / un-check) ===== -->
<dialog id="edit-dialog">
  <form id="edit-form" method="dialog">
    <h2 id="edit-title">Edit</h2>

    <div id="edit-items" class="picker-items"></div>

    <label class="field" id="edit-weight-field" hidden>Weight
      <input id="edit-weight" type="number" inputmode="decimal" min="0" step="0.1">
    </label>

    <label class="field">Time
      <input id="edit-time" type="datetime-local" required>
    </label>

    <div class="dialog-buttons">
      <button type="button" id="edit-uncheck" class="secondary-btn danger">Un-check</button>
      <span class="spacer"></span>
      <button type="button" id="edit-cancel" class="secondary-btn">Cancel</button>
...
<dialog id="sched-dialog">
  <form id="sched-form" method="dialog">
    ...
    <label class="field" id="s-item-field">Item
      <select id="s-item"></select>
    </label>
    <label class="field" id="s-newitem-field" hidden>New name
      <input id="s-newitem" type="text" autocapitalize="words" placeholder="Type the new one's name">
    </label>
...
```bash
git add index.html
git commit -m "v2: rewrite index.html — checklist layout, picker/edit dialogs, drop Log tab and overdue banner"
```
...
- [ ] **Step 2.1: DELETE these v1-only blocks** (no longer referenced by any HTML/JS after the rewrite):

- the `/* ---------- quick-log category buttons ---------- */` comment line only (the `.category-buttons` grid rule and `.cat-btn` rules are KEPT — the picker's category buttons reuse `.cat-btn`; delete just the `.category-buttons` selector from the grid rule, making it `.picker-cats { ... }` per Step 2.2)
- `.pill`, `.pill.done`, `.pill.early`, `.pill.late`, `.pill.todo` (all status-pill rules, and the `/* status pills on the Today screen */` comment)
- `.log-btn` and `.log-btn:active`
- `.today-banner`, `.today-banner.warn`, `.today-banner.ok` (and the `/* ---------- today nudge banner ---------- */` comment)

- [ ] **Step 2.2: ADD these blocks** (place after the `.sched-time` rule):

```css
/* ---------- checklist bars (Today) ---------- */
.check-row { cursor: pointer; }
.check-wrap { display: flex; flex-direction: column; align-items: center; gap: 3px; flex: none; }
.check {
...
.progress-line { color: var(--muted); font-size: 15px; margin: 2px 2px 12px; font-weight: 600; }
.add-row { justify-content: center; color: var(--green); font-weight: 600; font-size: 17px; min-height: 56px; }

/* ---------- picker dialog ---------- */
.picker-cats { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-bottom: 14px; }
.picker-cats .cat-btn { min-height: 64px; }
.picker-items { display: flex; flex-direction: column; margin-bottom: 8px; }
.picker-item {
  display: flex; align-items: center; gap: 12px;
  min-height: var(--tap); font-size: 17px; cursor: pointer;
  border-bottom: 1px solid var(--border); padding: 4px 2px;
}
.picker-item:last-child { border-bottom: none; }
.picker-item input { width: 24px; height: 24px; flex: none; }
.picker-item .dose { color: var(--muted); font-size: 13px; display: block; }
```

- [ ] **Step 2.3: Change the old category-buttons grid rule.** Replace:

```css
.category-buttons { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; }
```

with nothing (delete it — `.picker-cats` in Step 2.2 replaces it). Keep `.cat-btn` and `.cat-btn:active` unchanged.

- [ ] **Step 2.4: Commit**

```bash
git add styles.css
git commit -m "v2: checklist bar, green check, and picker styles; remove banner/pill/log-btn styles"
```
...
**Files:**
- Modify: `app.js` (full rewrite below)

Key design points baked into this code:
- Schedule rows: `{ id, time, category, item, group, note }` — `group: true` (Patches/Oils) means "pick items when logging."
- Entries: `{ id, category, items: [...], timestamp, amount, scheduleId }`. Legacy v1 entries (single `item`, `location`, `notes`, water `amount`) stay readable via `entryItems()` and History's legacy handling. Never migrated destructively.
- `migrate()` reseeds the schedule once (`cht.version` < 2 → seed v2, set version 2).
- No overdue logic anywhere.

- [ ] **Step 3.1: Replace the entire contents of `app.js` with:**
...
function openEdit(entry) {
  ...
  if (conf.group) {
    ...
    itemsFor(entry.category).forEach(it => {
    ...
    selected.forEach(name => {
      if (!itemsFor(entry.category).some(it => it.name === name)) {
...
```bash
git add app.js
git commit -m "v2: one-tap checklist logging — per-item bars, editable timestamps, group multi-select, no overdue"
```

---

### Task 4: Cache bump + README

**Files:**
- Modify: `service-worker.js:4`
- Modify: `README.md`

- [ ] **Step 4.1: In `service-worker.js`, change:**

```js
const CACHE_VERSION = "cht-v4";
```
...
- [ ] **Step 4.2: In `README.md`, update the stale descriptions:**

Replace this paragraph (near the end of the "Reminders" section):
...
Replace the "For the developer" bullet:
...
git commit -m "v2: bump cache to cht-v5, update README for checklist flow"
```

---

### Task 5: Manual verification pass (before pushing)

**Files:** none (verification only)

- [ ] **Step 5.1: Serve locally**

```bash
cd "D:/Users/brian/Projects/Personal/christinas-health-tracker"
python -m http.server 8000
```

Open http://localhost:8000 in a browser (device-emulation at iPhone width, e.g. 390px).

- [ ] **Step 5.2: Seed v1 data to prove the migration + legacy History.** In the browser console:

```js
localStorage.clear();
localStorage.setItem("cht.entries", JSON.stringify([
  { id: "old1", category: "water", item: "Light water", timestamp: "2026-07-06T08:15",
    amount: 8, location: null, notes: "old note", scheduleId: "sched-0-0630" },
  { id: "old2", category: "patches", item: "X39", timestamp: "2026-07-06T06:45",
    amount: null, location: "left shoulder", notes: "", scheduleId: null }
]));
localStorage.setItem("cht.schedule", JSON.stringify([
  { id: "sched-0-0630", time: "06:30", label: "Old v1 combined row", note: "", category: "water" }
]));
location.reload();
```

Verify: Today shows the NEW per-item default schedule (19 bars, not the old v1 row) — the migration reseeded it. History shows both v1 entries with their item names, the note, and "left shoulder."

- [ ] **Step 5.3: Walk every flow.** Each must behave exactly as stated:

1. Welcome popup appears (new copy, no "＋ Log" bullet). Dismiss with "Don't show again"; reload → doesn't reappear.
2. Tap "Light water 6:30" bar → instant big green ✓ with current time under it; progress line increments; NO dialogs.
3. Tap that ✓ bar again → edit box: change time to earlier, Save → new time shows under ✓.
4. Tap it again → Un-check → circle returns, progress decrements.
5. Tap the Patches bar → multi-select opens; tap Done with nothing selected → nothing happens, stays open; select X39 + Aeon → Done → one ✓, sub-line "X39, Aeon".
6. Tap the checked Patches bar → pick-list shows X39 + Aeon checked → add Energy, Save → sub-line updates.
7. In the picker, type a new name in "Add one not on the list" → Done → it's included, and reappears in the pick-list next open.
8. Tap "＋ Log something else" → category grid → Water → pick "Alkaline water" → Done → appears under "Also logged today."
9. "＋ Log something else" → Weight → empty Done → gentle message; enter 142 → Done → logged, shows "142" in History.
10. No red anywhere, no "overdue" text anywhere, at any time of day (set a bar's time to 00:01 via Schedule to force a long-passed unchecked bar and confirm it looks identical to future ones).
11. Schedule tab: add a new supplements row (category + item dropdowns work, "➕ Add a new one…" works), edit a row's time, delete a row, Reset to default.
12. More → Export backup → file downloads; re-import it → entries restored.
13. Schedule → Calendar button → .ics downloads; open it in a text editor → SUMMARY lines show item names (e.g. "SUMMARY:Light water"), one VEVENT per bar.
14. Toggle OS dark mode → all new elements readable in both themes.
15. Console shows zero errors throughout.

- [ ] **Step 5.4: Fix anything that fails, re-run the failing checks, and commit fixes:**

```bash
git add -A
git commit -m "v2: fixes from manual verification"
```

(Skip this commit if nothing failed.)
...
- [ ] **Step 6.1: Confirm `v1-backup` exists on GitHub** (already pushed earlier — verify):

```bash
git ls-remote --heads origin v1-backup
```

Expected: one line ending in `refs/heads/v1-backup`.
...
- [ ] **Step 6.3: Verify the live site.** Find the Pages URL:

```bash
gh api repos/rvpholley-dot/christinas-health-tracker/pages --jq .html_url
```

Open that URL in a browser; hard-refresh; confirm the new checklist Today screen loads and the console shows the new service worker (cht-v5) installing. (GitHub Pages can take 1–2 minutes to rebuild.)

- [ ] **Step 6.4: Tell Brian** the deploy is live, that Christina's phone will pick it up on next open (may need one close-and-reopen for the new service worker), and the rollback command if ever needed:

```bash
git push origin v1-backup:main --force
```
...
- **Spec coverage:** one-bar-per-item (Task 3 §6 + DEFAULT_SCHEDULE), tap-to-check (quickLog), editable timestamp + un-check + group item adjust (§8), group multi-select with one ✓ (§7), no overdue (nothing renders it; verification step 10), ounces dropped (no amount field outside weight), meals absent, ad-hoc ＋ bar (§6/§7), custom items kept (picker + schedule editor), History legacy support (§4/§9), schedule editor with category+item (§10), reseed migration (§2 migrate), backup (v1 backup restore keeps v2 schedule; entries restore), .ics uses rowTitle, welcome copy updated, cache bump, v1-backup branch, deploy + rollback. No gaps found.
- **Type consistency:** `entryItems()`, `rowTitle()`, `itemsFor()`, `pickerRow()` defined in Task 3 and used only there; IDs in Task 3 all exist in Task 1's HTML (checked: picker-*, edit-*, s-category, s-item, s-newitem-field, today-progress).
- **Removed-style check:** classes deleted in Task 2 (`pill`, `log-btn`, `today-banner`, `category-buttons`) are not referenced by Task 1 HTML or Task 3 JS.
